...
const TIPOS_AVALIACAO = [
  {
    ...
    match: (av, ds) => {
      const a = normStr(av);
      const d = normStr(ds);
      return a.includes("saepe") && !d.includes("ide");
    },
  },
  {
    key: "SAEB",
    ...
    match: (av, ds) => {
      const a = normStr(av);
      const d = normStr(ds);
      return a.includes("saeb") && !d.includes("ide");
    },
  },
  {
    key: "IDEB",
    ...
    match: (av, ds) => {
      const a = normStr(av);
      const d = normStr(ds);
      return a.includes("ideb") || d.includes("ideb");
    },
  },
  {
    key: "IDEPE",
    ...
    match: (av, ds) => {
      const a = normStr(av);
      const d = normStr(ds);
      return a.includes("idep") || d.includes("idep");
    },
  },
  {
    key: "Fluencia",
    ...
    match: (av, ds) => {
      const a = normStr(av);
      const d = normStr(ds);
      return a.includes("fluenc") || d.includes("fluenc");
    },
  },
];

// ==========================================================
// ÍCONES SVG DOS TIPOS
// ==========================================================
...
// ==========================================================
const GraficoDeBarras = ({ titulo, data, metas }) => {
  // ⚠️ Todos os hooks ANTES de qualquer return condicional (Rules of Hooks)
  const [hoveredPoint, setHoveredPoint] = useState(null);

  const getMetaParaAno = (ano) => {
    if (!Array.isArray(metas)) return null;
    const found = metas.find((m) => m.ano === Number(ano));
    return found ? Number(found.meta) : null;
  };

  const latestMetaRecord = useMemo(() => {
    if (!Array.isArray(metas) || metas.length === 0) return null;
    return [...metas].sort((a, b) => b.ano - a.ano)[0];
  }, [metas]);
  ...
  const isIdeb =
    ...
    (primeira.disciplina && (
      primeira.disciplina.toLowerCase().includes("ideb") || 
      primeira.disciplina.toLowerCase().includes("idepe") || 
      primeira.disciplina.toLowerCase().includes("idep")
    ));
  ...
  const isFluencia =
    ...
    (primeira.disciplina && (
      primeira.disciplina.toLowerCase().includes("fluencia") || 
      primeira.disciplina.toLowerCase().includes("fluência")
    ));
  ...
  // useMemo SEMPRE chamados — nunca depois de return condicional
  ...
  return (
    <div className={styles.graficoCard}>
      <div className={styles.graficoHeader}>
        ...
        {latestMetaRecord != null && latestMetaRecord.meta != null && (
          <span className={styles.metaBadge}>
            Meta ({latestMetaRecord.ano}): <strong>{Number(latestMetaRecord.meta).toFixed(2)}</strong>
          </span>
        )}
      </div>
      <div className={styles.chartOuterWrapper}>
        <div className={styles.chartWrapper}>
          <svg className={styles.svgChart} viewBox={`0 0 ${svgWidth} ${svgHeight}`} width="100%" height="100%">
            ...
            {latestMetaRecord != null && latestMetaRecord.meta != null && maxValor > 0 && (() => {
              const metaVal = Number(latestMetaRecord.meta);
              const metaValor = Math.min(metaVal, maxValor);
              const metaY = margin.top + chartHeight - (metaValor / maxValor) * chartHeight;
              return (
                <g>
                  <line x1={margin.left} y1={metaY} x2={svgWidth - margin.right} y2={metaY} className={styles.metaLine} />
                  <text x={margin.left + 10} y={metaY - 6} className={styles.metaLineLabel}>Meta ({latestMetaRecord.ano}): {metaVal.toFixed(2)}</text>
                </g>
              );
            })()}
            ...
            {svgPoints.map((pt) => {
              ...
              return (
                <g key={`bar-${pt.ano}`}>
                  <rect x={barX} y={pt.y} width={barWidth} height={barHeight} rx={rx} ry={rx} fill={`url(#grad-${pt.index % colors.length})`} className={`${styles.barRect} ${hoveredPoint?.index === pt.index ? styles.barRectActive : ""}`} />
                  {barHeight > rx && (
                    <rect x={barX} y={margin.top + chartHeight - rx} width={barWidth} height={rx} fill={`url(#grad-${pt.index % colors.length})`} />
                  )}
                </g>
              );
            })}
            ...
            {svgPoints.map((pt) => (
              <text key={`x-${pt.ano}`} x={pt.x} y={svgHeight - 15} textAnchor="middle" className={styles.xLabelText}>{pt.ano}</text>
            ))}

            {svgPoints.map((pt) => (
              <g key={`node-${pt.ano}`} className={`${styles.chartNodeGroup} ${hoveredPoint?.index === pt.index ? styles.chartNodeGroupActive : ""}`}>
            ...
            {svgPoints.map((pt) => (
              <rect key={`hover-${pt.ano}`} x={pt.x - chartWidth / N / 2} y={margin.top} width={chartWidth / N} height={chartHeight} fill="transparent" style={{ cursor: "pointer" }} onMouseEnter={() => setHoveredPoint(pt)} onMouseLeave={() => setHoveredPoint(null)} />
            ))}
          </svg>
          ...
          {hoveredPoint && (
            <div className={styles.htmlTooltip} style={{ left: `${(hoveredPoint.x / svgWidth) * 100}%`, top: `${(hoveredPoint.y / svgHeight) * 100 - 16}%` }}>
              ...
              {(() => {
                const metaParaAno = getMetaParaAno(hoveredPoint.ano);
                if (metaParaAno == null) return null;
                return (
                  <div className={styles.tooltipBody}>
                    <span className={styles.tooltipLabel}>Meta ({hoveredPoint.ano}):</span>
                    <strong className={styles.tooltipValue} style={{ color: "#f59e0b" }}>
                      {metaParaAno.toFixed(2)}
                    </strong>
                  </div>
                );
              })()}
    ...
    <nav className={styles.breadcrumb}>
      ...
      {tipo && (
        <>
          ...
          <button
            className={`${styles.breadcrumbBtn} ${!disciplina ? styles.breadcrumbActive : ""}`}
            onClick={disciplina ? onClickTipo : undefined}
            style={!disciplina ? { cursor: "default" } : {}}
          >
            {tipoLabel}
          </button>
        </>
      ...
      {disciplina && (
        <>
          <span className={styles.breadcrumbSep}>›</span>
          <button
            className={`${styles.breadcrumbBtn} ${!serie ? styles.breadcrumbActive : ""}`}
            onClick={serie ? onClickDisciplina : undefined}
            style={!serie ? { cursor: "default" } : {}}
          >
            {disciplina}
          </button>
        </>
...
export default function DetalhesEscola({ escola, onVoltar }) {
  ...
  async function fetchResultados() {
    setLoading(true);
    try {
      const [{ data: resultData, error: resErr }, { data: metasData, error: metasErr }] = await Promise.all([
        supabase.from("resultados").select("*").eq("escola_id", escola.id).order("ano", { ascending: true }),
        supabase.from("metas").select("*").eq("escola_id", escola.id),
      ]);
  ...
  const handleSaveEdit = async (e) => {
    e.preventDefault();
    if (!canEdit || !editingResultado) return;
  ...
  // Opções da etapa 2: disciplinas disponíveis para o tipo selecionado
  ...
  // Opções da etapa 3: séries disponíveis para tipo + disciplina
  ...
  // Grupo final selecionado para exibir o gráfico
  ...
  const printSummary = useMemo(() => {
    ...
    grupos.forEach((g) => {
      const latestRes = g.dados[g.dados.length - 1];
      if (!latestRes) return;
      
      const metaSerie = latestRes.serie.toString().toLowerCase().includes("2") ? "2"
        : latestRes.serie.toString().toLowerCase().includes("5") ? "5"
        : latestRes.serie.toString().toLowerCase().includes("9") ? "9"
        : "";
      ...
      summary.push({
        ...
        serie: latestRes.serie,
        disciplina: latestRes.disciplina,
        ano: latestRes.ano,
        resultado: latestRes.valor_indice,
        meta: matchMeta ? Number(matchMeta.meta) : null,
      });
    });
    
    return summary.sort((a, b) => {
      const sA = String(a.serie), sB = String(b.serie);
      const avA = String(a.avaliacao), avB = String(b.avaliacao);
      const dA = String(a.disciplina), dB = String(b.disciplina);
  ...
  const printByType = useMemo(() => {
    ...
    grupos.forEach((g) => {
      ...
      map[tipo.key].grupos.push({
        ...
        dadosPorAno: Object.fromEntries(g.dados.map((d) => [d.ano, Number(d.valor_indice)])),
        meta: metaRec ? Number(metaRec.meta) : null,
        metaAno: metaRec ? metaRec.ano : null,
    ...
    <div className={styles.pageContainer}>
      ...
      <header className={styles.stickyHeader}>
        ...
        <h1 className={styles.schoolTitle}>
          <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" style={{ color: "var(--primary-color)", flexShrink: 0 }}>
      ...
      <div className={styles.dashboardWorkspace}>
        ...
        <div className={styles.actionBar}>
          <div className={styles.actionBarLeft}>
            ...
            <button onClick={() => window.print()} className={styles.actionButton} style={{ marginLeft: "10px" }}>
        ...
        {/* ===================================================
            NAVEGAÇÃO DRILL-DOWN EM 3 ETAPAS
            =================================================== */}
        <div className={styles.drillContainer}>

          {/* Breadcrumb (aparece a partir do passo 2) */}
          {selectedTipo && (
            <Breadcrumb
              tipo={selectedTipo}
            ...
            <>
              {resultados.length === 0 ? (
                <div className={styles.drillEmpty}>
                  ...
                  {canEdit && <small>Use o botão "Novo Lançamento" acima para cadastrar o primeiro resultado.</small>}
                </div>
              ) : (
                <>
          ...
          <div className={`${styles.modalContainer} ${styles.modalContainerLarge}`} onClick={(e) => e.stopPropagation()}>
            <div className={styles.modalHeader}>
              <h3>
                ...
                Notas Lançadas ({resultados.length})
              </h3>
      ...
      {/* ==================================================
          MODAL: NOVO LANÇAMENTO (ADMIN APENAS)
          ================================================== */}
        ...
        <div className={styles.modalOverlay} onClick={() => setIsNovoLancamentoOpen(false)}>
          <div className={styles.modalContainer} onClick={(e) => e.stopPropagation()}>
            ...
            <form onSubmit={handleAddResultado}>
              <div className={styles.modalBody}>
                {/* Avaliação — agora é select com as 3 opções */}
                <div className={styles.modalFormGroup}>
                  <label className={styles.filterLabel} htmlFor="form-avaliacao">Tipo de Avaliação</label>
                  <select
                    id="form-avaliacao"
                    value={avaliacao}
                ...
                <div className={styles.modalFormGroup} style={{ marginTop: "16px" }}>
                  ...
                  <input
                    id="form-disciplina"
                    type="text"
                    placeholder="Ex: Matemática, Língua Portuguesa, Leitura"
                    value={disciplina}
                ...
                <div className={styles.modalFormGrid} style={{ marginTop: "16px" }}>
                  <div className={styles.modalFormGroup}>
                    <label className={styles.filterLabel} htmlFor="form-ano">Ano Civil</label>
                    <input
                      id="form-ano"
                      type="number"
                      placeholder="Ex: 2025"
                  ...
                  <div className={styles.modalFormGroup}>
                    <label className={styles.filterLabel} htmlFor="form-serie">Série / Ano Escolar</label>
                    <input
                      id="form-serie"
                      type="text"
                      placeholder="Ex: 5° Ano, 9° Ano"
                      value={serie}
                ...
                <div className={styles.modalFormGroup} style={{ marginTop: "16px" }}>
                  <label className={styles.filterLabel} htmlFor="form-valor">Valor do Índice</label>
                  <input
                    id="form-valor"
                    type="number"
                    step="0.01"
      ...
      {/* ==================================================
          MODAL: EDITAR RESULTADO (ADMIN APENAS)
          ================================================== */}
      {editingResultado && canEdit && (
            ...
            <form onSubmit={handleSaveEdit}>
              <div className={styles.modalBody}>
                <div className={styles.modalFormGroup}>
                  ...
                  <select
                    value={editingResultado.avaliacao}
                    onChange={(e) => setEditingResultado((prev) => ({ ...prev, avaliacao: e.target.value }))}
                    className={styles.filterSelect}
                    required
                  >
                    <option value="">Selecione...</option>
                ...
                <div className={styles.modalFormGroup} style={{ marginTop: "16px" }}>
                  ...
                  <input
                    ...
                    value={editingResultado.disciplina}
                    onChange={(e) => setEditingResultado((prev) => ({ ...prev, disciplina: e.target.value }))}
                    className={styles.filterInput}
                    required
                  />
                </div>
                ...
                <div className={styles.modalFormGrid} style={{ marginTop: "16px" }}>
                  <div className={styles.modalFormGroup}>
                    ...
                    <input
                      type="number"
                      placeholder="Ex: 2025"
                      value={editingResultado.ano}
                      onChange={(e) => setEditingResultado((prev) => ({ ...prev, ano: e.target.value }))}
                      className={styles.filterInput}
                      required
                    />
                  </div>
                  <div className={styles.modalFormGroup}>
                    ...
                    <input
                      type="text"
                      placeholder="Ex: 5° Ano, 9° Ano"
                      value={editingResultado.serie}
                      onChange={(e) => setEditingResultado((prev) => ({ ...prev, serie: e.target.value }))}
                      className={styles.filterInput}
                      required
                    />
                  </div>
                ...
                <div className={styles.modalFormGroup} style={{ marginTop: "16px" }}>
                  ...
                  <input
                    ...
                    step="0.01"
                    placeholder="Ex: 6.42"
                    value={editingResultado.valor_indice}
                    onChange={(e) => setEditingResultado((prev) => ({ ...prev, valor_indice: e.target.value }))}
                    className={styles.filterInput}
                    required
                  />
                </div>
      ...
      {/* ==================================================
          A4 PRINT REPORT LAYOUT (Visible only when printing)
          ================================================== */}
      <div className={styles.printReport}>

        {/* Cabeçalho */}
        <div className={styles.printReportHeader}>
          <div className={styles.printReportHeaderMain}>
            <img src={logo} alt="SIMREC Logo" className={styles.printLogo} />
            <p>Sistema de Monitoramento e Reconhecimento Capoeiras</p>
          </div>
          <div className={styles.printReportHeaderDate}>
            <p>Relatório de Desempenho Escolar</p>
            <small>Gerado em: {new Date().toLocaleDateString("pt-BR")} às {new Date().toLocaleTimeString("pt-BR", { hour: "2-digit", minute: "2-digit" })}</small>
          </div>
        </div>
        ...
        <div className={styles.printSchoolInfoBox}>
          <h3>{escola?.nome_escola}</h3>
          <div className={styles.printSchoolMetaGrid}>
            <div><span>Código INEP:</span> <strong>{escola?.codigo_inep}</strong></div>
            <div><span>Total de lançamentos:</span> <strong>{resultados.length}</strong></div>
            <div><span>Anos com dados:</span> <strong>{[...new Set(resultados.map(r => r.ano))].sort().join(", ") || "—"}</strong></div>
          </div>
        </div>

        {/* Seções por tipo de avaliação */}
        {printByType.length === 0 ? (
          <p className={styles.printNoData}>Nenhum indicador registrado para esta escola.</p>
        ) : (
          printByType.map((tipo, tipoIdx) => (
            <div
              key={tipo.label}
              className={styles.printSection}
              style={tipoIdx > 0 ? { pageBreakBefore: "auto" } : {}}
            >
              <div className={styles.printSectionTitleRow}>
                <h4>{tipo.label}</h4>
                <span className={styles.printSectionSubtitle}>
                  {tipo.label === "IDEB" && "Índice de Desenvolvimento da Educação Básica (Nacional)"}
                  {tipo.label === "IDEPE" && "Índice de Desenvolvimento da Educação de Pernambuco (Estadual)"}
                  {tipo.label === "Fluência" && "Avaliação de Fluência e Alfabetização (Anos Iniciais)"}
                  {tipo.label === "SAEPE" && "Sistema de Avaliação Educacional de Pernambuco"}
                  {tipo.label === "SAEB" && "Sistema de Avaliação da Educação Básica (Nacional)"}
                </span>
              </div>
              ...
              <table className={styles.printTable}>
                <thead>
                  <tr>
                    ...
                    <th>Disciplina</th>
                    {tipo.anos.map((ano) => (
                      <th key={ano} style={{ textAlign: "right" }}>{ano}</th>
                    ))}
                ...
                <tbody>
                  {tipo.grupos
                    .sort((a, b) => String(a.serie).localeCompare(String(b.serie), "pt-BR") || String(a.disciplina).localeCompare(String(b.disciplina), "pt-BR"))
                    .map((g) => {
                      const isPoints = tipo.label === "SAEPE" || tipo.label === "SAEB";
                      const fmt = (v) => v != null ? (isPoints ? `${Number(v).toFixed(0)} pts` : Number(v).toFixed(2)) : "—";
                      return (
                        <tr key={g.chave}>
                          <td>{g.serie}</td>
                          <td>{g.disciplina || "Geral"}</td>
                          {tipo.anos.map((ano) => (
                            <td key={ano} style={{ textAlign: "right", fontWeight: g.dadosPorAno[ano] != null ? "700" : "400", color: g.dadosPorAno[ano] != null ? "#0f172a" : "#94a3b8" }}>
                              {fmt(g.dadosPorAno[ano])}
                            </td>
                          ))}
                          <td style={{ textAlign: "right" }}>
                            {g.meta != null ? (
                              <>
                                <strong style={{ color: "#78350f" }}>{fmt(g.meta)}</strong>
                                <span style={{ display: "block", fontSize: "0.65rem", color: "#94a3b8", fontWeight: "500" }}>meta de {g.metaAno}</span>
                              </>
                            ) : "—"}
                          </td>
                        </tr>
        ...
        <div className={styles.printFooterBlock}>
          <p className={styles.printFooterNotice}>
            Este relatório foi gerado automaticamente pelo SIMREC — Sistema de Monitoramento e Reconhecimento Capoeiras. Os dados exibidos refletem os lançamentos registrados até a data de geração. Para informações completas e gráficos interativos, acesse o portal SIMREC.
          </p>
        </div>
      </div>
    </div>
